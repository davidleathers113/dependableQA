...
> **Pending production apply: `0016_ai_spend_metering`.** This migration (the `apply_call_processing_debit` RPC + its idempotency index) is applied to the **local** stack and committed, but **not yet applied to production** — it requires an explicit, controlled apply (Supabase CLI/MCP). It is additive and safe to apply schema-first (the code that uses it is not deployed while auto-deploy is paused, and the debit is best-effort, so the app stays correct either way). Apply it before deploying the Phase 5 code, then regenerate `supabase/types.ts` to confirm parity.
