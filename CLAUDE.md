...
- **Netlify functions** (`netlify/functions/**`): scheduled `ai-dispatch-scheduled` (2 min) and `ringba-api-sync-scheduled` (5 min); protected `ai-dispatch` / `import-dispatch` / `integration-ingest` / `stripe-webhook` authenticate via shared-secret header (timing-safe compare in `src/server/netlify-request.ts`) or provider signature. Details + header names: [docs/operations.md](docs/operations.md).
