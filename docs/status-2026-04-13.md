...
> **Point-in-time snapshot (2026-04-13).** This is a dated readiness assessment, not a living document — details may have drifted since. For current architecture see [`architecture.md`](architecture.md); the open risks called out here are also summarized in [operations](operations.md#known-operational-risks).
