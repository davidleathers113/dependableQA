...
describe("POST /api/imports/dispatch", () => {
  ...
  it("dispatches the batch metadata-only by default (no paid AI on import)", async () => {
  ...
  it("queues AI on import only when analyzeOnImport is explicitly true", async () => {
    const adminClient = { name: "admin" };
    requireApiSession.mockResolvedValue({
      user: { id: "user_1" },
      organization: { id: "org_1" },
    ...
    const response = await POST(createApiContext({
      request: new Request("http://localhost/api/imports/dispatch", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ batchId: "batch_1", analyzeOnImport: true }),
      }),
    }));
  ...
  it("treats a non-boolean analyzeOnImport as the safe (metadata-only) default", async () => {
    const adminClient = { name: "admin" };
    requireApiSession.mockResolvedValue({
      user: { id: "user_1" },
      organization: { id: "org_1" },
    ...
    const response = await POST(createApiContext({
      request: new Request("http://localhost/api/imports/dispatch", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ batchId: "batch_1", analyzeOnImport: "yes" }),
      }),
    }));
